...
function buildGraph() {
  ...
  interLinks.forEach(([source, target]) => {
    if (
      nodes.find((node) => node.id === source) &&
      nodes.find((node) => node.id === target)
    ) {
      links.push({ source, target, inter: true });
    }
  });
...
const SkillsGraph2D = ({ darkMode }) => {
  ...
  useEffect(() => {
    ...
    const sim = forceSimulation(nodes)
      .force(
        ...
        forceLink(links)
          .id((node) => node.id)
          .distance((link) => (link.inter ? 120 : link.source.isHub ? 65 : 45))
          .strength((link) => (link.inter ? 0.15 : 0.7)),
      )
      .force(
        "charge",
        forceManyBody().strength((node) => (node.isHub ? -300 : -50)),
      )
  ...
  const draw = useCallback(() => {
    ...
    links.forEach((link, index) => {
      ...
      const isHighlighted = highlightedLinks.has(index);
